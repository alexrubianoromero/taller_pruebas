...
function muestre_items($orden,$tabla,$conexion,$id_empresa)
     ...
     while($items = mysql_fetch_array($consulta_items))
			 ...
			 $i++;
    			...
    			<td width="82"><h7><div align = "right">'.number_format($items['valor_unitario'], 0, ',', '.').'</div></h7></td>
   			    <td width="85"><h7><div align = "right">'.number_format($items['total_item'], 0, ',', '.').'</div></h7></td>
					</tr>
				';
				//<td width="34">'.$i.'</td>
